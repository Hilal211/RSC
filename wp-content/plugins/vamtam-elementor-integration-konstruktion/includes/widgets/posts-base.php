...
if ( vamtam_theme_supports( 'posts-base--display-categories', 'posts-base--display-tags' , 'posts-base--read-more-theme-style' ) ) {
	...
	trait Vamtam_PostsBase_Classic_Skin_Overrides  {
		...
		protected function render_read_more_theme_style() {
			?>
				<div class="vamtam-post-thumb-wrap">
					<?php $this->render_thumbnail(); ?>
					<div class="vamtam-read-more-wrap">
						<a class="vamtam-read-more-icon" href="<?php echo esc_url( $this->current_permalink ); ?>">
							<?php \Elementor\Icons_Manager::render_icon( [
							'value' => 'vamtamtheme- vamtam-theme-plus' ,
							'library' => 'theme-icons'
							], [ 'aria-hidden' => 'true' ] ); ?>
						</a>
		...
		protected function render_post() {
			$use_read_more_theme_style = vamtam_theme_supports( 'posts-base--read-more-theme-style' ) && ! empty( $this->get_instance_value( 'show_read_more' ) ) && ! empty( $this->get_instance_value( 'use_read_more_theme_style' ) ) && $this->get_instance_value( 'thumbnail' ) === 'top';

			$this->render_post_header();

			if ( $use_read_more_theme_style ) {
				$this->render_read_more_theme_style();
			} else {
...
function update_image_controls_for_skin( $controls_manager, $widget, $skin_prefix ) {
	...
	if ( vamtam_theme_supports( 'posts-base--read-more-theme-style' ) ) {
		// Image Position.
		\Vamtam_Elementor_Utils::add_control_options( $controls_manager, $widget, "{$skin_prefix}_thumbnail", [
			'render_type' => 'template',
		] );
		// Image Spacing.
		\Vamtam_Elementor_Utils::add_control_options( $controls_manager, $widget, "{$skin_prefix}_image_spacing", [
			'selectors' => [
...
if ( vamtam_theme_supports( 'posts-base--horizontal-layout' ) ) {
	...
	function section_design_layout_before_section_end_classic_skin( $widget, $args ) {
		$controls_manager = \Elementor\Plugin::instance()->controls_manager;
	...
	function section_design_layout_before_section_end_vamtam_classic_skin( $widget, $args ) {
		$controls_manager = \Elementor\Plugin::instance()->controls_manager;
	...
	if ( \Vamtam_Elementor_Utils::is_widget_mod_active( 'posts' ) ) {
		...
		add_action( 'elementor/element/posts/classic_section_design_layout/before_section_end', __NAMESPACE__ . '\section_design_layout_before_section_end_classic_skin', 10, 2 );
		add_action( 'elementor/element/posts/vamtam_classic_section_design_layout/before_section_end', __NAMESPACE__ . '\section_design_layout_before_section_end_vamtam_classic_skin', 10, 2 );
	}
	if ( \Vamtam_Elementor_Utils::is_widget_mod_active( 'archive-posts' ) ) {
		// the posts widget has a --grid-column-gap var already
		add_action( 'elementor/element/archive-posts/archive_classic_section_design_layout/before_section_end', __NAMESPACE__ . '\section_design_layout_before_section_end_classic_skin', 10, 2 );
		add_action( 'elementor/element/archive-posts/vamtam_classic_section_design_layout/before_section_end', __NAMESPACE__ . '\section_design_layout_before_section_end_vamtam_classic_skin', 10, 2 );
	}
}
...
if ( vamtam_theme_supports( 'posts-base--read-more-theme-style' ) ) {
	function add_read_more_theme_style_controls_for_skin( $controls_manager, $widget, $skin_prefix ) {
		$widget->start_injection( [
            'of' => "{$skin_prefix}_read_more_spacing",
        ] );
		// Use Theme Style.
		$widget->add_control(
			"{$skin_prefix}_use_read_more_theme_style",
			[
				'label' => __( 'Use Theme Style', 'vamtam-elementor-integration' ),
				'type' => $controls_manager::SWITCHER,
				'prefix_class' => 'vamtam-has-',
				'return_value' => 'read-more-theme-style',
				'default' => ( $skin_prefix === 'vamtam_classic' ) ? 'read-more-theme-style' : '',
				'conditions' => [
					'terms' => [
						[
							'name' => "{$skin_prefix}_show_read_more",
							'operator' => '===',
							'value' => 'yes',
						]
						],
...
if ( vamtam_theme_supports( [ 'posts-base--title-underline-anim', 'posts-base--read-more-theme-style' ] ) ) {
	...
	function section_design_content_before_section_end_classic_skin( $widget, $args ) {
		...
		if ( vamtam_theme_supports( 'posts-base--read-more-theme-style' ) ) {
			add_read_more_theme_style_controls_for_skin( $controls_manager, $widget, 'classic' );
		}
	}
	...
	function section_design_content_before_section_end_vamtam_classic_skin( $widget, $args ) {
		...
		if ( vamtam_theme_supports( 'posts-base--read-more-theme-style' ) ) {
			add_read_more_theme_style_controls_for_skin( $controls_manager, $widget, 'vamtam_classic' );
		}
	}
	// Theme Settings.
	if ( \Vamtam_Elementor_Utils::is_widget_mod_active( 'posts' ) ) {
