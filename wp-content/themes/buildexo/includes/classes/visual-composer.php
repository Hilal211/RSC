...
class Visual_Composer {
	...
	function init() {
		...
		$maps = apply_filters( 'turner_vc_map', $this->maps );
		...
		foreach ( $maps as $value) {
			...
			if ( file_exists( $file ) ) {
				...
				if ( function_exists( 'wpturner_shortcode' ) ) {

					$tag = esc_attr( turner_set( $data, 'base' ) );



					wpturner_shortcode( $tag, array( $this, 'output' ) );
	...
	function output( $atts, $content = null, $tag ) {
		...
		turner_template_load( 'shortcodes/' . $tag . '.php', compact( 'atts', 'content', 'tag' ) );
	...
	function get_file( $tag ) {
		...
		$file = turner_template( 'includes/resource/vc_map/'.$tag . '.php' );

		$file = apply_filters( "turner_vc_map_file_{$tag}", $file );
	...
	function toggle( $settings, $value ) {
		...
		$values = isset( $settings['value'] ) && is_array( $settings['value'] ) ? $settings['value'] : array( esc_html__( 'Yes', 'buildexo' ) => 'true' );
