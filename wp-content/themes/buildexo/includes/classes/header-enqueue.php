...
class Header_Enqueue {
	...
	public static function scripts() {
		...
		$options = get_theme_mod( 'buildexo' . '_options-mods' );
		...
		$scripts = apply_filters( 'TURNER/includes/classes/header_enqueue/scripts', $scripts );
		...
		$header_data = array(
			...
			'nonce'   => wp_create_nonce( TURNER_NONCE ),

		);



		wp_localize_script( 'jquery', 'turner_data', $header_data );



		if ( turner_set( $options, 'footer_js' ) ) {



			wp_add_inline_script( 'jquery', turner_set( $options, 'footer_js' ) );
	...
	public static function styles() {
		...
		$styles = apply_filters( 'TURNER/includes/classes/header_enqueue/styles', $styles );
		...
		foreach ( $styles as $name => $style ) {
			...
			if ( strstr( $style, 'http' ) || strstr( $style, 'https' ) || strstr( $style, 'fonts.googleapis' ) ) {
				...
				wp_enqueue_style( "turner-{$name}", $style );

			} else {

				wp_enqueue_style( "turner-{$name}", get_template_directory_uri() . '/' . $style );
		...
		if ( $custom_font = $options->get('theme_custom_font') ) {
            ...
            $header_styles .= turner_custom_fonts_load( $custom_font );

        }



		wp_add_inline_style( 'turner-main-style', $header_styles );
	...
	public static function resource_hints( $urls, $relation_type ) {
		...
		if ( wp_style_is( 'turner-fonts', 'queue' ) && 'preconnect' === $relation_type ) {
	...
	public static function header_styles() {
		...
		$settings = get_theme_mod( 'buildexo' . '_options-mods' );



		if ( $custom_font = turner_set( $settings, 'theme_custom_font' ) ) {



			$styles .= apply_filters('turner_redux_custom_fonts_load', $custom_font );
